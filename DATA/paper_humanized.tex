...
\begin{abstract}
Subsea power and telecommunication cables carry essential international data and energy traffic. When acoustic sensors register anomalous seafloor vibrations, operators cannot tell immediately whether a surface ship caused the disturbance or whether the cable was actually harmed. Standard monitoring approaches tend to treat acoustic spikes as direct evidence of surface threats, leading either to nuisance alarms or to blind spots when vessel-broadcast telemetry is manipulated. In this work, we separate physical disturbance detection, spatio-temporal tracking, vessel kinematics, sensor health diagnostics, and counter-evidence evaluation into distinct streams. A fail-closed decision hierarchy fuses these streams into operational states \dstate{T0}--\dstate{T3}, routing compromised or ambiguous cases to a degraded-evidence state \dstate{TX}. We tested the approach across three submarine distributed acoustic sensing (DAS) records: an Automatic Identification System (AIS)-linked container-ship passage in the Belgian North Sea (Marlinks), a quiescent deep-sea background recording in the Ionian Sea (EMSO Western Ionian), and a 27.625-day Arctic deployment (Dryad Oliktok). On the Marlinks transit, coherent DAS strain separates 1~km corridor crossings with an area under the receiver operating characteristic curve (AUC) of $0.959$ (95\% CI $[0.887, 1.000]$), while raw acoustic power tracks vessel range at Spearman $\rho=-0.948$. Under quiet and environmental excitation, the system yielded zero false escalations across 85/85 deep-sea windows and all 215 Arctic hourly windows, tracking sea-surface wave height over a stable noise floor (drift $+0.0188/\mathrm{hr}$, $p=0.8037$). Against four vessel-side AIS spoofing families across five severities, the adversarial evasion rate remained $0.0$. By design, our fail-closed logic trades unconstrained recall for safety: full fusion reaches lower escalation F1 ($0.462$ real, $0.235$ in 2{,}200 simulation trials) than unweighted baselines ($0.824$ and $0.333$). Pipeline latency averages 8.289~ms on standard hardware, comfortably clearing real-time operational constraints.
\end{abstract}
...
The empirical evaluation was conducted across three distinct real submarine DAS deployments, cataloged by cryptographic SHA-256 hashes in Table~\ref{tab:datasets} to guarantee end-to-end provenance.
...
Dataset A records an active commercial vessel transit across a submarine power export cable in the Belgian North Sea \cite{b12}. The recording comprises 60 consecutive 10-second windows across 250 spatial channels with synchronous 1D distance ground truth $y(t) \in [26.22, 2785.91]$~m for a 304~m container ship. The observed minimum distance 26.22~m denotes the closest recorded 1D ground-truth measurement; the kinematic CPA trajectory fit (Section~\ref{sec:kinematics}) yields a distinct fitted parameter $d_{\mathrm{cpa}} = 125.1$~m. Dataset A serves strictly as a 1D vessel proximity validation dataset, not a full 2D trajectory tracking benchmark.

\subsection{Dataset B: EMSO Western Ionian Seafloor Observatory}
Dataset B was collected by the European Multidisciplinary Seafloor and water-column Observatory (EMSO) off Eastern Sicily at 2{,}100~m water depth \cite{b11}. It contains 105 consecutive 10-second windows (10,500 temporal samples at 10~Hz) across 2{,}963 optical channels, capturing an unperturbed deep-sea acoustic noise floor devoid of nearby maritime traffic.

\subsection{Dataset C: Dryad Oliktok Arctic Submarine Cable}
Dataset C was recorded along a 31.1~km subsea telecommunications cable extending offshore from Oliktok Point, Alaska, into the Beaufort Sea \cite{b15}. The primary evaluated tensor (\texttt{oliktok\_das\_mooring\_hourly\_dataset.nc}) spans 215 hourly windows across 183 physical DAS channels and 32 frequency bins (0.0078--0.4941~Hz) over 27.625 continuous days (August 24 to September 20, 2023 UTC), accompanied by along-cable segments in \texttt{oliktok\_das\_hourly\_along\_cable\_dataset.nc}. Co-located seafloor mooring instruments recorded significant wave height ($H_s$), energy period ($T_e$), and dynamic bottom pressure variance. Dataset C contains no AIS broadcasts, vessel ground truth, or structural damage events; its role is strictly to assess long-duration environmental robustness and escalation-avoidance under natural hydrodynamic wave forcing.
...
\caption{Long-term temporal stability and environmental wave coupling on Dataset C (Dryad Oliktok, 27.625 days). Seafloor acoustic energy tracks independent ocean wave height without producing anomalous sensor drift.}
\label{fig:oliktok-temporal}
\end{figure}

Strain energy tracked external hydrodynamic forcing recorded by nearby oceanographic moorings: Spearman $\rho = +0.3141$ ($p = 2.63\times10^{-6}$, Pearson $r = +0.3370, p = 4.31\times10^{-7}$) against significant wave height $H_s$, and Spearman $\rho = +0.4951$ ($p = 1.07\times10^{-14}$, Pearson $r = +0.4903, p = 2.11\times10^{-14}$) against bottom pressure variance (Fig.~\ref{fig:oliktok-temporal}). These correlations demonstrate coupling to surface sea states rather than cable damage, which cannot be verified without physical incident records.

Running Dataset C through the fixed decision pipeline yielded 93 \dstate{T0} decisions (43.26\%) during calm periods, 122 \dstate{T1} decisions (56.74\%) during elevated wave forcing, and zero \dstate{T2}/\dstate{T3} vessel alerts (0.0\% environmental escalation rate; Table~\ref{tab:oliktok}). Because Dataset C lacks AIS or vessel logs, this test specifically demonstrates environmental stability and false-alarm suppression under persistent wave action, rather than vessel attribution accuracy.
...
\caption{Channel dropout robustness on real DAS. F1 remains constant while \dstate{TX} declarations rise: the system degrades by asking for help, not by guessing.}
...
\caption{Reliability calibration curve of fused score $R$ on the held-out transit leg (Brier score $=0.122$; 10-bin expected calibration error (ECE) $=0.224$). Marker area denotes bin sample occupancy.}
